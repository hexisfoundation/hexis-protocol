...
Core formula:
    ...
    C   Context Multiplier  [0.8, 1.25]  geographic justice correction
...
Security:
    ...
    - **C is computed from a self-declared country and is not verified.**
      Nothing checks it against the request, an IP, or anything else. It is
      the one trusted input in this formula, it is disclosed rather than
      hidden, and since 2026-08-17 its range and its lifetime are both bounded
      — see C_MIN / C_MAX and C_DECAY_HALFLIFE_MINTS below, and
      STEP4_PROPOSAL.md for how those numbers were arrived at.
"""
...
# ------------------------------------------------------------------
# The Context multiplier's range. [0.5, 2.0] until 2026-08-17.
#
# C is computed from a country string the actor types at registration, which
# nothing verifies. The old range let that string be worth 4x (0.5 -> 2.0) on
# every mint, forever. These two numbers are chosen, not inherited:
#
#   - Log-symmetric, so 1.25 = 1 / 0.8 exactly. The world median sits in the
#     true middle and no country is advantaged by the arithmetic of the clamp.
#     The old pair had this property too (2.0 = 1/0.5) and it is worth keeping.
#   - 1.5625x total span (1.25 / 0.8), down from 4x. The most a false
#     declaration can buy is 56% faster accrual instead of 300% faster.
#   - Combined with the decay applied at the mint site, the *lifetime* value of
#     the lie is bounded rather than perpetual: at a half-life of 100 mints the
#     integral of the boost is 100/ln2 x (C-1) ~= 36 events' worth of extra
#     HEXIS for the most-boosted country, against an unbounded +100% before.
#
# What this does not do is make the declaration true. It prices it. The
# disclosure in the docstring below, on /docs and in the whitepaper is the
# other half and is not optional: a protocol that asks everyone else to
# declare what they cannot prove has to say where it does the same.
# ------------------------------------------------------------------
C_MIN = 0.8
C_MAX = 1.25

# Slow decay of the boost, applied at the mint site (see BridgeState.
# mine_hexis_for_job), not here — the formula in whitepaper §5 is unchanged,
# exactly as the geo damping scale is applied without touching it.
#
# Half-life in an actor's own prior mints. 100 is chosen against the observed
# scale of a HEXIS career rather than by taste: the whitepaper puts the High
# grade at ~72 honest jobs, so a half-life of 100 spans the whole climb — the
# boost is worth most to a newcomer and least to an established actor.
#
# That shape is the equity argument taken seriously rather than diluted. The
# case for C is that a newcomer in a poor country needs a leg-up to reach a
# usable grade at all; it is not that an established worker deserves a
# permanent subsidy. A boost that decays is the first claim without the second.
C_DECAY_HALFLIFE_MINTS = 100.0
...
class HexisMiner:
    ...
    def calc_C(e: BehaviorEvent) -> float:
        """
        Context Multiplier = clamp(REFERENCE_GDP / COUNTRY_GDP, C_MIN, C_MAX)
        Range: [0.8, 1.25] since 2026-08-17. Was [0.5, 2.0].

        Rawlsian justice: same sacrifice costs more in poorer countries.
        High income (US, SG): C = 0.8 | World median: C = 1.0 | Low income: C = 1.25
        Unknown country → DEFAULT (world median).

        **The country is self-declared and is not verified.** Nothing checks it
        against the request, against an IP, or against anything else. That is
        the reason the range moved, and STEP4_PROPOSAL.md is why it moved to
        these two numbers rather than to some others.
        """
        code = str(e.actor_country or "").upper().strip()
        gdp = GDP_PER_CAPITA.get(code, GDP_PER_CAPITA["DEFAULT"])
        if gdp <= 0:
            return 1.0
        return max(C_MIN, min(C_MAX, REFERENCE_GDP / gdp))
